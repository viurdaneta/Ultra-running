library(readxl)
data <- read_excel("Documents/STA4164/Project B/ultrarunning.xlsx")
...
df$pb_surface <- factor(df$pb_surface, levels = c("trail", "track", "road", "mix"))
...
# Compute MAE and MSE
mae_value <- mae(df$teique_sf, predictions)
mse_value <- mse(df$teique_sf, predictions)
